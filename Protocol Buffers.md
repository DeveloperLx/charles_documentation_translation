<html>
 <body>
  <div class="content"> 
   ...
   <p>Charles希望<code>Content-Type</code> header中的<code>desc</code>参数，包含有指向FileDescriptorSet文件位置的有效的URL。它可以是一个<code>文件</code>或是<code>http</code>类型的URL。Charles会在每次选中某个Protobuf的视图时，尝试检索FileDescriptorSet，这意味着<code>desc</code>的URL只会在使用的时候，才会被读取。这在当大量protocol buffer被记录时，是非常有用的。</p> 
   <h4>FileDescriptorSet缓存<a id="caching" name="caching"></a></h4> 
